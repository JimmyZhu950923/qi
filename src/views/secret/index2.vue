        ...
        <el-tabs>
          <el-tab-pane label="详情">
            <el-row>
              ...
              <el-col :span="20">{{ secretName }}</el-col>
            </el-row><br>
            <el-row>
              <el-col :span="4">命名空间:</el-col>
              <el-col :span="20">{{ secretNamespace }}</el-col>
            </el-row><br>
            <el-row>
...
<script>
import { getSingle } from '@/api/secret'
export default {
  data() {
    return {
      loading: true,
  ...
  methods: {
    selS: function() {
      this.secretName = this.$route.params.name
      this.secretNamespace = this.$route.params.namespace
      getSingle(this.secretNamespace, this.secretName).then(response => {
        // debugger
        this.secretName = response.data.metadata.name
        this.secretNamespace = response.data.metadata.namespace
